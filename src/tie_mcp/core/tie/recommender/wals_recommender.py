...
class WalsRecommender(Recommender):
    ...
    def _update_factor(
        ...
        if p != data.shape[0]:
            raise ValueError(f"Data row count mismatch: opposing_factors p={p}, data.shape[0]={data.shape[0]}")
        if q <= 0:
            raise ValueError(f"q must be > 0 (got {q})")
        if alpha <= 0:
            raise ValueError(f"alpha must be > 0 (got {alpha})")
        if regularization_coefficient < 0:
            raise ValueError(f"regularization_coefficient must be >= 0 (got {regularization_coefficient})")
        ...
        def V_T_C_I_V(V, c_array):
            ...
            for i in nonzero_c:
                ...
                if square_addition.shape != (k, k):
                    raise RuntimeError(
                        f"square_addition shape mismatch: expected {(k, k)}, got {square_addition.shape}"
                    )
        ...
        for i in range(q):
            ...
            if C_u.shape != (p,):
                raise ValueError(f"C_u shape mismatch: expected {(p,)}, got {C_u.shape}")

            confidence_scaled_v_transpose_v = V_T_C_I_V(V, C_u)

            # X = (V^T CV + \lambda I)^{-1} V^T CP
            inv = np.linalg.inv(V_T_V + confidence_scaled_v_transpose_v + regularization_coefficient * np.identity(k))
    ...
    def _coerce_input_matrix(self, data: Any, expect_shape: tuple[int, int]) -> np.ndarray:
        ...
        if hasattr(data, "indices") and hasattr(data, "values"):
            if not _TF_AVAILABLE:
                raise ImportError("TensorFlow required to pass a SparseTensor to WalsRecommender (not installed)")
            dense = tf.sparse.to_dense(tf.sparse.reorder(data)).numpy()  # type: ignore[attr-defined]
        else:
            if not isinstance(data, np.ndarray):
                raise TypeError("data must be either a TensorFlow SparseTensor or a numpy ndarray")
            dense = data
        if dense.shape != expect_shape:
            raise ValueError(f"Input matrix shape mismatch: expected {expect_shape}, got {dense.shape}")
        return dense
    ...
    def fit(
        ...
        for _ in range(epochs):
            # step 1: update U
            self._U = self._update_factor(self._V, P.T, alpha, regularization_coefficient)
    ...
    def evaluate(
        ...
        if hasattr(test_data, "indices") and hasattr(test_data, "values"):
            if not _TF_AVAILABLE:
                raise ImportError("TensorFlow required to evaluate with a SparseTensor (not installed)")
            ...
            if test_data.shape != (self.m, self.n):
                raise ValueError(f"test_data shape mismatch: expected {(self.m, self.n)}, got {test_data.shape}")
    ...
    def predict_new_entity(
        ...
        **kwargs,
        ...
        if hasattr(entity, "indices") and hasattr(entity, "values"):
            if not _TF_AVAILABLE:
                raise ImportError("TensorFlow required for SparseTensor entity input (not installed)")
            ...
            if not isinstance(entity, np.ndarray):
                raise TypeError("entity must be a TensorFlow SparseTensor or a numpy ndarray")
            dense_entity = entity

        if dense_entity.shape != (self.n,):
            raise ValueError(f"entity shape mismatch: expected {(self.n,)}, got {dense_entity.shape}")
        ...
        if new_entity_factor.shape != (1, self._U.shape[1]):
            raise ValueError(
                f"new_entity_factor shape mismatch: expected {(1, self._U.shape[1])}, got {new_entity_factor.shape}"
            )
