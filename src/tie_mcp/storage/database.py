...
from sqlalchemy import String, Text, DateTime, Float, Integer, Boolean, JSON, Index
...
class DatabaseManager:
    ...
    async def set_default_model(self, model_id: str):
        ...
        try:
            async with self.get_session() as session:
                ...
                current_default = await session.query(Model).filter(Model.is_default == True).first()
                if current_default:
    ...
    async def get_default_model(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            async with self.get_session() as session:
                model = await session.query(Model).filter(Model.is_default == True).first()
                if model:
                    return self._model_to_dict(model)
                return None
